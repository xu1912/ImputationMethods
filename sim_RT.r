...
for(i in 1:ns){
	...
	if(model_name == "RT"){
		...
		finalize_workflow(select_best(rpart_grid,metric = "rmse"))
      	...
      	# * 6) last_fit() ----
      	fit <- extract_spec_parsnip(final_rpart_workflow) %>%
       	fit(Y_res ~ ., data = rd)
	}
	...
	if(model_name == "RT"){
		...
		final_rpart_workflow <- rpart_workflow %>%
		finalize_workflow(select_best(rpart_grid,metric = "accuracy"))

      	# * 6) last_fit() ----
      	fit1_resp <- extract_spec_parsnip(final_rpart_workflow) %>%
       	fit(r_idx ~ ., data = rdp)
	...
	for (k in 1:K) {
		...
		if(model_name == "RT"){
			...
			finalize_workflow(select_best(rpart_grid,metric = "rmse"))
      		...
      		# * 6) last_fit() ----
      		g_model <- extract_spec_parsnip(final_rpart_workflow) %>%
			...
			finalize_workflow(select_best(rpart_grid,metric = "accuracy"))
	      	...
	      	# * 6) last_fit() ----
      		e_model <- extract_spec_parsnip(final_rpart_workflow) %>%
