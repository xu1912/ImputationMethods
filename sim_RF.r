...
ns=100
K=5
mn=3              ##indicates the data generation model 1-3
...
for(i in 1:ns){
	...
	if(model_name == "RF"){
		...
		dat_folds <- vfold_cv(rd,v = 10)
      	...
      	# * 4) tune_grid() ----
      	set.seed(123)
      	randforest_cube <- grid_space_filling(
        		finalize(mtry(), rd[,-1]),
          		trees(),
        		min_n(),
        		size = 30
		...
		finalize_workflow(select_best(randforest_grid,metric = "rmse"))
      	...
      	# * 6) last_fit() ----
      	fit <- extract_spec_parsnip(final_randforest_workflow) %>%
       	fit(Y_res ~ ., data = rd)
	}
	...
	if(model_name == "RF"){
		...
		final_randforest_workflow <- randforest_workflow %>%
		finalize_workflow(select_best(randforest_grid,metric = "accuracy"))

      	# * 6) last_fit() ----
      	fit1_resp <- extract_spec_parsnip(final_randforest_workflow) %>%
       	fit(r_idx ~ ., data = rdp)
	...
	for (k in 1:K) {
		...
		if(model_name == "RF"){
			...
			finalize_workflow(select_best(randforest_grid,metric = "rmse"))
      		...
      		# * 6) last_fit() ----
      		g_model <- extract_spec_parsnip(final_randforest_workflow) %>%
			...
			finalize_workflow(select_best(randforest_grid,metric = "accuracy"))
	      	...
	      	# * 6) last_fit() ----
      		e_model <- extract_spec_parsnip(final_randforest_workflow) %>%
