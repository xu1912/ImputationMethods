...
for(i in 1:ns){
	...
	if(model_name == "KNN"){
		...
		final_knn_workflow <- knn_workflow %>%
		finalize_workflow(select_best(knn_grid,metric = "accuracy"))

      	# * 6) last_fit() ----
      	fit1_resp <- extract_spec_parsnip(final_knn_workflow) %>%
       	fit(r_idx ~ ., data = rdp)

	}
	pv=predict(fit1_resp, data.frame(X),type="prob")
	pv$.pred_Y[pv$.pred_Y==0]=0.01
	Y_est_2=sum(r_idx/pv$.pred_Y*Y)/N
	...
	for (k in 1:K) {
		...
		if(model_name == "KNN"){
			...
			finalize_workflow(select_best(knn_grid,metric = "rmse"))
      		...
      		# * 6) last_fit() ----
      		g_model <- extract_spec_parsnip(final_knn_workflow) %>%
			...
			finalize_workflow(select_best(knn_grid,metric = "accuracy"))
      		...
      		# * 6) last_fit() ----
      		e_model <- extract_spec_parsnip(final_knn_workflow) %>%
